...
src = source[:3141]+op[1:-16]+source[5308:30592]+ed[:-28]+source[32748:]

cred_op = source[3141:5308] # this is the OP with credits
cred_ed = source[30592:32748] # this is the ED with credits
...
op_fin = atf.ApplyCredits(cred_op, op[1:-16], deband[3141:5308])
op_fin = CoolDegrain(op_fin, thsad=16, thsadc=48, blksize=8, overlap=4)

ed_fin = atf.ApplyCredits(cred_ed, ed[:-28], deband[30592:32748])
ed_fin = CoolDegrain(ed_fin, thsad=16, thsadc=48, blksize=8, overlap=4)

mrg = deband[:3141]+op_fin+deband[5308:30592]+ed_fin+deband[32748:]
