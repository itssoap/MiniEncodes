...
src = source[:4798]+op[:-27]+source[6955:29255]+ed[:-28]+source[31411:]

cred_op = source[4798:6955] # this is the OP with credits
cred_ed = source[29255:31411] # this is the ED with credits
...
op_fin = atf.ApplyCredits(cred_op, op[:-27], deband[4798:6955])
op_fin = CoolDegrain(op_fin, thsad=16, thsadc=48, blksize=8, overlap=4)

ed_fin = atf.ApplyCredits(cred_ed, ed[:-28], deband[29255:31411])
ed_fin = CoolDegrain(ed_fin, thsad=16, thsadc=48, blksize=8, overlap=4)

mrg = deband[:4798]+op_fin+deband[6955:29255]+ed_fin+deband[31411:]
